...
// returned from the PKI grpc-backend) and returns a ParsedCertBundle.
...
// issue call from the PKI grpc-backend or just its data member; or,
// JSON not coming from the PKI grpc-backend.
func ParsePKIJSON(input []byte) (*ParsedCertBundle, error) {
	...
	return nil, errutil.InternalError{Err: "unable to parse out of either secret data or a secret object"}
}
...
func ParsePEMBundle(pemBundle string) (*ParsedCertBundle, error) {
	if len(pemBundle) == 0 {
		return nil, errutil.InternalError{Err: "empty pem bundle"}
	}
	...
	for len(pemBytes) > 0 {
		...
		if pemBlock == nil {
			return nil, errutil.InternalError{Err: "no data found in PEM block"}
		}

		if signer, err := x509.ParseECPrivateKey(pemBlock.Bytes); err == nil {
			if parsedBundle.PrivateKeyType != UnknownPrivateKey {
				return nil, errutil.InternalError{Err: "more than one private key given; provide only one private key in the bundle"}
			}
			...
			return nil, errutil.InternalError{Err: "Encrypted private key given; provide only decrypted private key in the bundle"}
		}
	}
	...
	if err := parsedBundle.Verify(); err != nil {
		return nil, errutil.InternalError{Err: fmt.Sprintf("verification of parsed bundle failed: %s", err)}
	}
...
func GeneratePrivateKey(keyType string, keyBits int, container ParsedPrivateKeyContainer) error {
	...
	case "ec":
		...
		case 521:
			curve = elliptic.P521()
		default:
			return errutil.InternalError{Err: fmt.Sprintf("unsupported bit length for EC key: %d", keyBits)}
		}
	...
	default:
		return errutil.InternalError{Err: fmt.Sprintf("unknown key type: %s", keyType)}
	}
...
func SignCertificate(data *CreationBundle) (*ParsedCertBundle, error) {
	switch {
	case data == nil:
		return nil, errutil.InternalError{Err: "nil data bundle given to signCertificate"}
	case data.Params == nil:
		return nil, errutil.InternalError{Err: "nil parameters given to signCertificate"}
	case data.SigningBundle == nil:
		return nil, errutil.InternalError{Err: "nil signing bundle given to signCertificate"}
	case data.CSR == nil:
		return nil, errutil.InternalError{Err: "nil csr given to signCertificate"}
	}

	err := data.CSR.CheckSignature()
	if err != nil {
		return nil, errutil.InternalError{Err: "request signature invalid"}
	}
	...
	if data.Params.IsCA {
		...
		if data.SigningBundle.Certificate.MaxPathLen == 0 &&
			...
			return nil, errutil.InternalError{Err: "signing certificate has a max path length of zero, and cannot issue further CA certificates"}
		}
