...
// with the PKI grpc-backend but can be generally useful. Functionality
...
//
// Functionality specific to the PKI grpc-backend includes some types
...
import (
	...
	"encoding/pem"
	"fmt"
	"github.com/go-various/helper/errutil"
	"math/big"
	"net"
	"net/url"
	"strings"
	...
	"github.com/go-various/helper/errwrap"
)
...
// Secret is used to attempt to unmarshal a  secret
...
func (c *CertBundle) ToParsedCertBundle() (*ParsedCertBundle, error) {
	...
	if len(c.PrivateKey) > 0 {
		...
		if pemBlock == nil {
			return nil, errutil.InternalError{Err: "Error decoding private key from cert bundle"}
		}
		...
		case PKCS8Block:
			...
			if err != nil {
				return nil, errutil.InternalError{Err: fmt.Sprintf("Error getting key type from pkcs#8: %v", err)}
			}
		...
		default:
			return nil, errutil.InternalError{Err: fmt.Sprintf("Unsupported key block type: %s", pemBlock.Type)}
		}

		result.PrivateKey, err = result.getSigner()
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Error getting signer: %s", err)}
		}
	}
	...
	if len(c.Certificate) > 0 {
		...
		if pemBlock == nil {
			return nil, errutil.InternalError{Err: "Error decoding certificate from cert bundle"}
		}
		result.CertificateBytes = pemBlock.Bytes
		result.Certificate, err = x509.ParseCertificate(result.CertificateBytes)
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Error encountered parsing certificate bytes from raw bundle: %v", err)}
		}
	}
	switch {
	case len(c.CAChain) > 0:
		for _, cert := range c.CAChain {
			...
			if err != nil {
				return nil, errutil.InternalError{Err: fmt.Sprintf("Error encountered parsing certificate bytes from raw bundle via CA chain: %v", err)}
			}
	...
	case len(c.IssuingCA) > 0:
		...
		if pemBlock == nil {
			return nil, errutil.InternalError{Err: "Error decoding ca certificate from cert bundle"}
		}

		parsedCert, err := x509.ParseCertificate(pemBlock.Bytes)
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Error encountered parsing certificate bytes from raw bundle via issuing CA: %v", err)}
		}
...
func (p *ParsedCertBundle) getSigner() (crypto.Signer, error) {
	...
	if p.PrivateKeyBytes == nil || len(p.PrivateKeyBytes) == 0 {
		return nil, errutil.InternalError{Err: "Given parsed cert bundle does not have private key information"}
	}
	...
	case ECBlock:
		...
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Unable to parse CA's private EC key: %s", err)}
		}

	case PKCS1Block:
		signer, err = x509.ParsePKCS1PrivateKey(p.PrivateKeyBytes)
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Unable to parse CA's private RSA key: %s", err)}
		}
	...
	case PKCS8Block:
		if k, err := x509.ParsePKCS8PrivateKey(p.PrivateKeyBytes); err == nil {
			...
			default:
				return nil, errutil.InternalError{Err: "Found unknown private key type in pkcs#8 wrapping"}
			}
		}
		return nil, errutil.InternalError{Err: fmt.Sprintf("Failed to parse pkcs#8 key: %v", err)}
	default:
		return nil, errutil.InternalError{Err: "Unable to determine type of private key; only RSA and EC are supported"}
	}
	return signer, nil
}
...
func getPKCS8Type(bs []byte) (PrivateKeyType, error) {
	...
	if err != nil {
		return UnknownPrivateKey, errutil.InternalError{Err: fmt.Sprintf("Failed to parse pkcs#8 key: %v", err)}
	}
	...
	default:
		return UnknownPrivateKey, errutil.InternalError{Err: "Found unknown private key type in pkcs#8 wrapping"}
	}
}
...
func (c *CSRBundle) ToParsedCSRBundle() (*ParsedCSRBundle, error) {
	...
	if len(c.PrivateKey) > 0 {
		...
		default:
			...
			if _, err := x509.ParseECPrivateKey(pemBlock.Bytes); err == nil {
				...
				return nil, errutil.InternalError{Err: fmt.Sprintf("Unknown private key type in bundle: %s", c.PrivateKeyType)}
			}
		}
	...
	if len(c.CSR) > 0 {
		...
		if err != nil {
			return nil, errutil.InternalError{Err: fmt.Sprintf("Error encountered parsing certificate bytes from raw bundle via CSR: %v", err)}
		}
	}
